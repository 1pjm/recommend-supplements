...
# 영양소별 권장 섭취량 산출
energy_avg, carbohydrate_avg, protein_avg, fat_avg, calcium_avg, phosphorus_avg, iron_avg, thiamine_avg, riboflavin_avg, niacin_avg, vitamin_c_avg = calculate_average_requirements(gender, age, pa_index, weight, height)
...
print("7)인(mg): {:.2f}".format(phosphorus_avg))
print("8)티아민(mg): {:.2f}".format(thiamine_avg) if thiamine_avg is not None else "8)티아민: 성별 또는 연령에 따른 값 없음")
print("9)리보플라빈(mg): {:.2f}".format(riboflavin_avg) if riboflavin_avg is not None else "9)리보플라빈: 성별 또는 연령에 따른 값 없음")
print("10)니아신(mg NE): {:.2f}".format(niacin_avg) if niacin_avg is not None else "10)니아신: 성별 또는 연령에 따른 값 없음")
print("11)비타민C(mg): {:.2f}".format(vitamin_c_avg) if vitamin_c_avg is not None else "11)비타민C: 성별 또는 연령에 따른 값 없음")
...
additional_file_path = "C:/Users/김하은/OneDrive/바탕 화면/식품영양성분DB_가공식품_20240416.xlsx"
...
for nutrient, diff in zip(['에너지(kcal)', '단백질(g)', '지방(g)', '탄수화물(g)', '칼슘(mg)', '철(mg)', '인(mg)', '티아민(mg)', '리보플라빈(mg)', '니아신(mg)', '비타민 C(mg)'], gap):
    if diff > 0:
